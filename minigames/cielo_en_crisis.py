...
def Lluvia():
    ...
    fondo = pygame.image.load(
        str(
            (ruta / ".." / "assets" / "CosasDeMinijuegos" / "FondoLluvia.png").resolve()
        )
    )
    ...
    VENTANA.blit(
        fuente.render(f"Jugador 2: {puntaje2}", True, BLANCO), (ANCHO - 220, 10)
    )
    ...
    VENTANA.blit(
        fuente.render(mensaje, True, BLANCO), (ANCHO // 2 - 200, ALTO // 2 - 30)
    )
    VENTANA.blit(
