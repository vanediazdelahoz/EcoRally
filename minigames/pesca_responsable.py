...
def Pesca():
    ...
    LATA_IMAGE = pygame.image.load(
        str(
            (
                ruta / ".." / "assets" / "CosasDeMinijuegos" / "Basura" / "Lata.png"
            ).resolve()
        )
    )
    ...
    def end_game():
        ...
        # Puntajes
        WIN.blit(
            fuente.render(f"Jugador 1: {players[0]['score']}", True, WHITE), (20, 10)
        )
        WIN.blit(
        ...
        WIN.blit(
            fuente.render(mensaje, True, WHITE), (WIDTH // 2 - 200, HEIGHT // 2 - 30)
        )
        resumen = f"Jugador 1: {s1}   Jugador 2: {s2}"
        WIN.blit(
            fuente.render(resumen, True, WHITE), (WIDTH // 2 - 200, HEIGHT // 2 + 30)
        )
