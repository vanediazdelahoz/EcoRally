...
class PescaResponsableState(State):
    def __init__(self, game):
        ...
        self.trash_spawn_frequency = 1  # Generar basura cada frame (como en la versión antigua)
    ...
    def load_images(self):
        ...
        try:
            # Cargar barcos
            self.BARCO1_IMAGE = pygame.image.load("assets/CosasDeMinijuegos/Barco/Barco Izquierda.png").convert_alpha()
            self.BARCO2_IMAGE = pygame.image.load("assets/CosasDeMinijuegos/Barco/Barco Derecha.png").convert_alpha()
        ...
        try:
            ...
            player1_img = pygame.image.load("assets/CosasDeMinijuegos/Barco/Cebo Izquierda.png").convert_alpha()
            player2_img = pygame.image.load("assets/CosasDeMinijuegos/Barco/Cebo Derecha.png").convert_alpha()
            self.players[0]["image"] = pygame.transform.scale(player1_img, (self.PLAYER_SIZE, self.PLAYER_SIZE))
            self.players[1]["image"] = pygame.transform.scale(player2_img, (self.PLAYER_SIZE, self.PLAYER_SIZE))
        except Exception as e:
            print(f"Error cargando cebos: {e}")
            self.players[0]["image"] = pygame.Surface((self.PLAYER_SIZE, self.PLAYER_SIZE))
            self.players[0]["image"].fill((255, 255, 0))
            self.players[1]["image"] = pygame.Surface((self.PLAYER_SIZE, self.PLAYER_SIZE))
            self.players[1]["image"].fill((255, 255, 0))
        
        try:
            # Cargar imágenes de basura
            aluminio_img = pygame.image.load("assets/CosasDeMinijuegos/Basura/Aluminio.png").convert_alpha()
            botella_img = pygame.image.load("assets/CosasDeMinijuegos/Basura/Botella.png").convert_alpha()
            lata_img = pygame.image.load("assets/CosasDeMinijuegos/Basura/Lata.png").convert_alpha()
            
            self.ALL_TRASH_IMAGES = [
                pygame.transform.scale(aluminio_img, (self.TRASH_SIZE, self.TRASH_SIZE)),
    ...
    def _end_minigame(self):
        ...
        if len(self.game.state_stack) >= 2:
            ...
            if hasattr(board_game, 'continue_after_minigame'):
                board_game.continue_after_minigame(self.players[0]["score"], self.players[1]["score"])
    ...
    def update(self, dt):
        ...
        if self.game_state == "PLAYING":
            ...
            for player in self.players:
                self.handle_player_input(player, keys_pressed)
            ...
            self.trash_list[:] = [t for t in self.trash_list if -self.TRASH_SIZE <= t["rect"].x <= SCREEN_WIDTH]
    ...
    def render(self, screen):
        ...
        texto1 = self.font.render(f"{self.player1_name}: {self.players[0]['score']}", True, WHITE)
        screen.blit(texto1, (20, 10))
        
        # Puntaje jugador 2 (esquina derecha) - CORREGIDO
        texto2 = self.font.render(f"{self.player2_name}: {self.players[1]['score']}", True, WHITE)
