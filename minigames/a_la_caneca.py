...
def Clasificar():
    ...
    fondo = pygame.image.load(
        str(
            (
                ruta / ".." / "assets" / "CosasDeMinijuegos" / "FondoClasificar.png"
            ).resolve()
        )
    )
    ...
    imagenes = {
        n: pygame.transform.scale(
            pygame.image.load(
                str(
                    (
                        ruta / ".." / "assets" / "CosasDeMinijuegos" / "Basura" / n
                    ).resolve()
                )
            ),
            (80, 80),
    ...
    def dibujar():
        ...
        for tipo, pos in canecas_j1.items():
            VENTANA.blit(canecas_img[tipo], pos)
    ...
    def mostrar_overlay_ganador():
        ...
        VENTANA.blit(
            fuente.render(mensaje, True, BLANCO), (cuadro.x + 50, cuadro.y + 50)
        )
        marcador = f"Jugador 1: {puntaje1}   Jugador 2: {puntaje2}"
        VENTANA.blit(
            fuente.render(marcador, True, BLANCO), (cuadro.x + 50, cuadro.y + 100)
        )
