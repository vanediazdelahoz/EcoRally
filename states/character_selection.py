...
class CharacterSelection(State):
    def __init__(self, game):
        ...
        self.controls_offset_y = (
            80  # Distancia vertical desde el panel hacia ABAJO (ajustable)
        )
        ...
        self.font_controls = load_font(
            "assets/fonts/mc-ten-lowercase-alt.ttf", self.controls_font_size
        )

        # Título CON SOMBREADO GRIS
        self.title_surface = self._render_text_with_shadow(
            self.font_title, "SELECCIONA TU PERSONAJE", WHITE
        )
        self.title_rect = self.title_surface.get_rect(
            center=(SCREEN_WIDTH // 2, self.title_y_position)
        )
        ...
        try:
            flecha_original = pygame.image.load(
                "assets/images/landscape/flecha.png"
            ).convert_alpha()
            ...
            # Controles Player 1
                ...
                if not self.confirmed_p1:
                    ...
                    if not self.confirmed_p2 or self.selected_p1 != self.final_p2:
                        ...
                        print(
                            f"Jugador 1 confirmó personaje {self.final_p1}: {self.character_names[self.final_p1]}"
                        )
                        ...
                        if config.machine_mode:
                            ...
                            print(
                                f"Bot seleccionó automáticamente personaje {self.final_p2}: {self.character_names[self.final_p2]}"
                            )
                ...
                if event.key == pygame.K_LEFT and not self.confirmed_p2:
                    ...
                    if not self.confirmed_p2:
                        ...
                        if not self.confirmed_p1 or self.selected_p2 != self.final_p1:
                            ...
                            config.characters[1] = self.final_p2
                            print(
                                f"Jugador 2 confirmó personaje {self.final_p2}: {self.character_names[self.final_p2]}"
                            )
                            ...
                            print(
                                "Ese personaje ya fue seleccionado por el otro jugador."
                            )
                    else:
                        # Retractarse P2
                        if not self.both_confirmed:
